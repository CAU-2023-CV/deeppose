...
def get_model(model_path, n_joints, resul_dir, resume_model):
    model_fn = os.path.basename(model_path)
    model_name = model_fn.split('.')[0]
    model = importlib.load_source(model_name, model_path)
...
def transform(args, x_queue, datadir, fname_index, joint_index, o_queue):
    trans = Transform(args) #Transform ??
    while True:
        x=x_queue.get()
        if x is None:
            break
...
if __name__=='__main__':
    ...
    test_iter = iterators.MultiprocessIterator(test_dataset, args.batchsize, repeat=False, shuffle=False)

    gpus = [int(i) for i in args.gpus.aplit(',')] #gpus:0,1,2...
    devices = {'main':gpus[0]}
    if len(gpus)>2:
        for gid in gpus[1:]:
